<?php use Trusted\CryptoARM\Docs;
...
class CBPTrustedCAApprove extends CBPCompositeActivity implements
    IBPEventActivity,
    ...
    public function Subscribe(IBPActivityExternalEventListener $eventHandler) {
        ...
        $arParameters['CommentRequired'] = $this->IsPropertyExists('CommentRequired')
            ? $this->CommentRequired
            : 'N';
        $arParameters['AccessControl'] =
            $this->IsPropertyExists('AccessControl') && $this->AccessControl == 'Y' ? 'Y' : 'N';
        ...
        $this->taskId = $taskService->CreateTask(array(
            'USERS' => $arUsers,
        ...
        if (!$this->IsPropertyExists('SetStatusMessage') || $this->SetStatusMessage == 'Y') {
            ...
            $message =
                $this->IsPropertyExists('StatusMessage') && strlen($this->StatusMessage) > 0
                    ? $this->StatusMessage
                    : GetMessage('BPAA_ACT_INFO');
            $this->SetStatusTitle(
                str_replace(
                    array(
                        '#PERC#',
                    ...
                    array(
                        ...
                        $totalCount,
                        $totalCount,
                        GetMessage('BPAA_ACT_APPROVERS_NONE'),
                        GetMessage('BPAA_ACT_APPROVERS_NONE'),
                    ),
                    $message
                )
    ...
    public function OnExternalEvent($arEventParameters = array()) {
        ...
        if ($timeoutDuration > 0) {
            if (
                array_key_exists('SchedulerService', $arEventParameters) &&
                $arEventParameters['SchedulerService'] == 'OnAgent'
            ) {
                $this->IsTimeout = 1;
        ...
        if (
            !array_key_exists('USER_ID', $arEventParameters) ||
            intval($arEventParameters['USER_ID']) <= 0
        ) {
            return;
        }
        ...
        if ($arUser = $dbUser->Fetch()) {
            $this->Comments =
                ...
                $arUser['LOGIN'] .
                '): ' .
                ($approve ? GetMessage('BPAA_LOG_Y') : GetMessage('BPAA_LOG_N')) .
                "\n" .
                (strlen($arEventParameters['COMMENT']) > 0
                    ? GetMessage('BPAA_LOG_COMMENTS') . ': ' . $arEventParameters['COMMENT']
                    : '') .
                "\n";
        }
        ...
        if ($this->ApproveType == 'any') {
            ...
            if ($this->ApproveWaitForAll == 'Y') {
                ...
                $noneApprovedPercent =
                    (($this->VotedCount - $this->ApprovedCount) / $this->TotalCount) * 100;
                if (
                    $this->ApprovedPercent > $this->ApproveMinPercent ||
                    ($this->ApprovedPercent == 100 && $this->ApproveMinPercent == 100)
                ) {
                    ...
                    $noneApprovedPercent > 0 &&
                    $noneApprovedPercent >= 100 - $this->ApproveMinPercent
                ) {
                    $result = 'NonApprove';
                }
            }
        ...
        if ($result != 'Continue') {
            $this->taskStatus =
                $result == 'Approve' ? CBPTaskStatus::CompleteYes : CBPTaskStatus::CompleteNo;
            $this->Unsubscribe($this);
            $this->writeApproversResult();
    ...
    public static function ShowTaskForm($arTask, $userId, $userName = '') {
        ...
        if (
            !array_key_exists('ShowComment', $arTask['PARAMETERS']) ||
            $arTask['PARAMETERS']['ShowComment'] != 'N'
        ) {
            $required = '';
            ...
            $form .=
                ...
                (strlen($arTask['PARAMETERS']['CommentLabelMessage']) > 0
            ...
            if ($doc->getStatus() === DOC_STATUS_BLOCKED) {
                ...
                $blockMessage =
                    GetMessage('BPAA_ACT_BLOCK_BY') .
                    Docs\Utils::getUserName(CUser::GetByID($blockUser));
                $form .= "<tr><td>$blockMessage</td><td></td></tr>";
                if ($blockUser == $USER->GetID()) {
                    $form .=
                        '<tr><td colspan="2"><input type="button" class="bp-button bp-button-transparent" style="border: none" value="' .
                        GetMessage('BPAA_ACT_UNBLOCK') .
                        '" onclick="trustedCA.unblock([' .
                        $lastDocId .
                        '], () => { location.reload() })" /></td></tr>';
                }
            } else {
                $form .=
                    '<tr><td colspan="2"><input type="button" class="bp-button bp-button-accept" style="border: none" value="' .
                    GetMessage('BPAA_ACT_SIGN') .
                    '" onclick="trustedCA.sign([' .
                    $lastDocId .
                    '], null, null)" /></td></tr>';
            }
        }
        ...
        $buttons =
            ...
            (strlen($arTask['PARAMETERS']['TaskButton1Message']) > 0
                ? $arTask['PARAMETERS']['TaskButton1Message']
                : GetMessage('BPAA_ACT_BUTTON1')) .
            '"/>' .
            '<input type="submit" name="nonapprove" value="' .
            (strlen($arTask['PARAMETERS']['TaskButton2Message']) > 0
    ...
    public static function PostTaskForm(
        $arTask,
        $userId,
        $arRequest,
        &$arErrors,
        ...
        try {
            ...
            $arEventParameters = array(
                ...
                'USER_NAME' => $userName,
                'COMMENT' => isset($arRequest['task_comment'])
                    ? trim($arRequest['task_comment'])
                    : '',
            );

            if (
                (isset($arRequest['approve']) && strlen($arRequest['approve']) > 0) ||
                (isset($arRequest['INLINE_USER_STATUS']) &&
                    $arRequest['INLINE_USER_STATUS'] == CBPTaskUserStatus::Yes)
            ) {
                $arEventParameters['APPROVE'] = true;
            } elseif (
                (isset($arRequest['nonapprove']) && strlen($arRequest['nonapprove']) > 0) ||
                (isset($arRequest['INLINE_USER_STATUS']) &&
                    $arRequest['INLINE_USER_STATUS'] == CBPTaskUserStatus::No)
            ) {
            ...
            if (
                ...
                ($arTask['PARAMETERS']['CommentRequired'] === 'Y' ||
                    ($arTask['PARAMETERS']['CommentRequired'] === 'YA' &&
                        $arEventParameters['APPROVE']) ||
                    ($arTask['PARAMETERS']['CommentRequired'] === 'YR' &&
                        !$arEventParameters['APPROVE']))
            ) {
                $label =
                    strlen($arTask['PARAMETERS']['CommentLabelMessage']) > 0
    ...
    public static function ValidateProperties(
        ...
        if (!array_key_exists('Users', $arTestProperties)) {
            ...
            foreach ($arTestProperties['Users'] as $userId) {
                if (
                    (!is_array($userId) && strlen(trim($userId)) > 0) ||
        ...
        if ($bUsersFieldEmpty) {
            $arErrors[] = array(
                'code' => 'NotExist',
                'parameter' => 'Users',
                'message' => GetMessage('BPAA_ACT_PROP_EMPTY1'),
            );
        }
        ...
        if (!array_key_exists('ApproveType', $arTestProperties)) {
            $arErrors[] = array(
                ...
                'message' => GetMessage('BPAA_ACT_PROP_EMPTY2'),
            );
        } else {
            if (!in_array($arTestProperties['ApproveType'], array('any', 'all', 'vote'))) {
                $arErrors[] = array(
                    'code' => 'NotInRange',
                    'parameter' => 'ApproveType',
                    'message' => GetMessage('BPAA_ACT_PROP_EMPTY3'),
                );
            }
        }

        if (
            !array_key_exists('Name', $arTestProperties) ||
            strlen($arTestProperties['Name']) <= 0
        ) {
            $arErrors[] = array(
                'code' => 'NotExist',
                'parameter' => 'Name',
                'message' => GetMessage('BPAA_ACT_PROP_EMPTY4'),
            );
        }
    ...
    private function CalculateTimeoutDuration() {
        ...
        $timeoutDurationType = $this->IsPropertyExists('TimeoutDurationType')
            ? $this->TimeoutDurationType
            : 's';
    ...
    public static function GetPropertiesDialog(
                ...
                foreach ($arMap as $k => $v) {
                    if (array_key_exists($k, $arCurrentActivity['Properties'])) {
                        if ($k == 'Users') {
                            ...
                            $arCurrentValues['timeout_duration'] =
                                $arCurrentActivity['Properties']['TimeoutDuration'];
                            if (
                        ...
                        if (
                            ...
                            !array_key_exists($arMap[$k], $arCurrentValues)
                        ) {
        ...
        return $runtime->ExecuteResourceFile(__FILE__, 'properties_dialog.php', array(
