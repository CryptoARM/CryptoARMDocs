...
if ($REQUEST_METHOD == "POST") {
    ...
    if (!$USER->CanDoFileOperation('fm_upload_file', $arPath)) {
        ...
        if ($nums > 0) {
            for ($i = 1; $i <= $nums; $i++) {
                ...
                elseif (!$arType && $arValue)
                    $strWarning .= Loc::getMessage("TR_CA_DOCS_UPLOAD_VALUE_WITHOUT_PROP_ERROR") . "\n";
                elseif (
        ...
        if (strlen($strWarning) <= 0) {
            $backurl = '/bitrix/admin/trusted_cryptoarm_docs.php?PAGEN_1=1&SIZEN_1=20&lang=' . LANGUAGE_ID;
            if (!empty($_POST["apply"])) LocalRedirect($ret); else LocalRedirect($ret);
        }
    }
}

$APPLICATION->SetTitle(Loc::getMessage("TR_CA_DOCS_UPLOAD_TITLE"));
                ...
                <table id="bx-upload-tbl">
                    <tr class="heading">
                        ...
                        <td style="text-align: left!important;">
                            <?= Loc::getMessage("TR_CA_DOCS_UPLOAD_FILE_PROP") ?>
                            <span class="required"><sup>1</sup></span>
                        </td>

                        <td style="text-align: left!important;">
                            <?= Loc::getMessage("TR_CA_DOCS_UPLOAD_FILE_VALUE") ?>
