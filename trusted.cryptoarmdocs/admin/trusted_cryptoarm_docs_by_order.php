...
$eventManager->addEventHandler('main', 'OnBeforeMailSend', function ($event) {
    ...
    foreach ($eventParams as $mailKey => $mailParams) {
        ...
        if ($mailParams['HEADER']['X-EVENT_NAME'] === 'TR_CA_DOCS_MAIL_BY_ORDER') {
            foreach ($mailParams['ATTACHMENT'] as $attachKey => $attachParam) {
                ...
                $eventParams[$mailKey]['ATTACHMENT'][$attachKey]['CONTENT_TYPE'] =
    ...
    foreach ($eventParams as $mailKey => $mailParams) {
        $event->addResult(
            new \Bitrix\Main\EventResult(\Bitrix\Main\EventResult::SUCCESS, $mailParams)
        );
    }
});
...
if (($arID = $lAdmin->GroupAction()) && $POST_RIGHT == 'W') {
    ...
    switch ($_REQUEST['action']) {
        ...
        case 'unblock':
            echo '<script>';
            echo 'window.parent.trustedCA.unblock(' .
                json_encode($ids) .
                ', () => { window.parent.' .
                $reloadTableJs .
                ' })';
        ...
        case 'remove':
            echo '<script>';
            echo 'window.parent.trustedCA.remove(' .
                json_encode($ids) .
                ', false, () => { window.parent.' .
                $reloadTableJs .
                ' })';
        ...
        case 'send_mail':
            ...
            if ($e > 0) {
                $message =
                    Loc::getMessage('TR_CA_DOCS_MAIL_ERROR_PRE') .
                    $e .
                    Loc::getMessage('TR_CA_DOCS_MAIL_ERROR_POST');
                echo "<script>alert('" . $message . "')</script>";
            }
...
while ($arRes = $rsData->NavNext(true, 'f_')) {
    ...
    if (!$emailStatus) {
        $docEmailIcon .=
            '<img src="/bitrix/themes/.default/icons/trusted.cryptoarmdocs/email_not_sent.png"';
        $docEmailIcon .= ' class="trca-email-icon">';
        $docEmailStatus .= Loc::getMessage('TR_CA_DOCS_EMAIL_NOT_SENT');
    } elseif ($emailStatus == 'SENT') {
        $docEmailIcon .=
            '<img src="/bitrix/themes/.default/icons/trusted.cryptoarmdocs/email_sent.png"';
        $docEmailIcon .= ' class="trca-email-icon">';
        $docEmailStatus .= Loc::getMessage('TR_CA_DOCS_EMAIL_SENT');
    } elseif ($emailStatus == 'READ') {
        $docEmailIcon .=
            '<img src="/bitrix/themes/.default/icons/trusted.cryptoarmdocs/email_read.png"';
        $docEmailIcon .= ' class="trca-email-icon">';
    ...
    foreach ($docList as $doc) {
        ...
        $docViewField .=
            "value='i' ondblclick='event.stopPropagation()' onclick='trustedCA.verify([";
        $docViewField .= $docId . "])' title='" . Loc::getMessage('TR_CA_DOCS_VERIFY_DOC') . "'/>";
        $docViewField .=
            "<a class='trca-tn-document' title='" .
            Loc::getMessage('TR_CA_DOCS_DOWNLOAD_DOC') .
            ' ';
        $docViewField .=
            Loc::getMessage('TR_CA_DOCS_OPEN_QUOTE') .
            $doc->getName() .
            Loc::getMessage('TR_CA_DOCS_CLOSE_QOUTE');
...
<form name="find_form" method="get" action="<?= $APPLICATION->GetCurPage() ?>">
    ...
    <tr>
        ...
        <td>
            <input type="text" name="find_order" size="47" value="<?= htmlspecialchars(
                $find_order
            ) ?>">
        </td>
    </tr>
    ...
    <tr>
        ...
        <td>
            <input type="text" name="find_clientEmail" size="47" value="<?= htmlspecialchars(
                $find_clientEmail
            ) ?>">
        </td>
    </tr>
    <tr>
        ...
        <td>
            <input type="text" name="find_clientName" size="47" value="<?= htmlspecialchars(
                $find_clientName
            ) ?>">
        </td>
    </tr>
    ...
    <tr>
        ...
        <td>
            ...
            $arr = array(
                ...
                'reference_id' => array('', 'NOT_SENT', 'SENT', 'READ'),
            );
            echo SelectBoxFromArray(
                'find_orderEmailStatus',
                $arr,
    ...
    <tr>
        ...
        <td>
            ...
            $arr = array(
                ...
                'reference_id' => array('', 'CLIENT', 'SELLER', 'BOTH', 'NONE'),
            );
            echo SelectBoxFromArray(
                'find_docState',
                $arr,
