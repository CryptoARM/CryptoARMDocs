...
$ret = "/bitrix/admin/trusted_cryptoarm_docs_by_user.php?lang=" . LANGUAGE_ID;
...
if ($REQUEST_METHOD == "POST") {
    ...
    if (!$USER->CanDoFileOperation('fm_upload_file', $arPath)) {
        ...
        if (strlen($strWarning) <= 0) {
            $backurl = '/bitrix/admin/trusted_cryptoarm_docs_by_user.php?lang=' . LANGUAGE_ID;
            if (!empty($_POST["apply"])) LocalRedirect($ret); else LocalRedirect($ret);
        }
    }
}

$APPLICATION->SetTitle(Loc::getMessage("TR_CA_DOCS_UPLOAD_BY_USER_TITLE"));
...
<? if (strlen($strWarning) <= 0 || $bCan): ?>
                ...
                <table id="bx-upload-tbl">
                    <tr class="heading">
                        ...
                        <td style="text-align: left!important;">
                            <?= Loc::getMessage("TR_CA_DOCS_UPLOAD_FILE_USER_ID") ?>
    ...
    <?echo BeginNote();?>
    <span class="required"><sup>*</sup></span><?echo Loc::getMessage("TR_CA_DOCS_UPLOAD_USER_ID_NOTE")?>
    <?echo EndNote();?>
