...
$moduleOptions = array(
    ...
    'EVENT_SIGNED_BY_CLIENT',
    'EVENT_SIGNED_BY_SELLER',
    'EVENT_SIGNED_BY_BOTH',
    'EVENT_SIGNED_BY_CLIENT_ALL_DOCS',
    'EVENT_SIGNED_BY_SELLER_ALL_DOCS',
    'EVENT_SIGNED_BY_BOTH_ALL_DOCS',
    'EVENT_EMAIL_SENT',
    'EVENT_EMAIL_READ',
    'MAIL_EVENT_ID_TO',
    'MAIL_TEMPLATE_ID_TO',
...
$daysInSelector = array(
    ...
    'REFERENCE_ID' => array(1, 3, 7, 14, 30, 99999),
);
...
?>
    ...
    <form method="POST" enctype="multipart/form-data"
        ...
        <tr>
            <td colspan="2">
                <?php echo BeginNote(),
                    Loc::getMessage('TR_CA_DOCS_DEFAULT_EMAIL_DESCRIPTION'),
                    EndNote(); ?>
            </td>
        </tr>
        ...
        <tr>
            ...
            <td>
                ...
                <option value="" <?= $MAIL_EVENT_ID_TO ? '' : 'selected' ?>>
                    <?= Loc::getMessage('TR_CA_DOCS_DEFAULT_EMAIL_NOT_SELECTED') ?>
                </option>
                    <?php
        ...
        <tr>
            ...
            <td>
                <select name="MAIL_TEMPLATE_ID_TO" id="MAIL_TEMPLATE_ID_TO">
                    <option value="" <?= $MAIL_TEMPLATE_ID_TO ? '' : 'selected' ?>>
                        <?= Loc::getMessage('TR_CA_DOCS_DEFAULT_EMAIL_NOT_SELECTED') ?>
                    </option>
                    <?php
                    $templates = CEventMessage::GetList($by = 'id', $order = 'asc', array(
                        'TYPE_ID' => $MAIL_EVENT_ID_TO,
                    ));
            ...
            <td style="display: flex; align-items: center;">
                <input id="LICENSE_ACCOUNT_NUMBER"
                       ...
                       placeholder="<?= Loc::getMessage(
                           'TR_CA_DOCS_LICENSE_INPUT_ACCOUNT_NUMBER_PLACEHOLDER'
                       ) ?>"
                       <?= $LICENSE_ACCOUNT_NUMBER !== '' ? "readonly='true'" : '' ?>
                       maxlength="16"
                       type="text"/>
                <div id="DIV_BTN_CREATE_NEW_ACCOUNT" <?= $LICENSE_ACCOUNT_NUMBER !== ''
                    ? 'hidden'
                    : '' ?>>
                    <input type="submit"
                    ...
                    <input type="button"
                           ...
                           value="<?= GetMessage(
                               'TR_CA_DOCS_LICENSE_CREATE_NEW_ACCOUNT_NUMBER'
                           ) ?>"/>
                </div>
                <div id="DIV_INPUT_ACCOUNT_NUMBER" <?= $LICENSE_ACCOUNT_NUMBER !== ''
                    ? ''
                    : 'hidden' ?>>
                    <input type="button"
        ...
        <tr>
            <td colspan="2">
                <?php echo BeginNote(),
                    Loc::getMessage('TR_CA_DOCS_EVENTS_DESCRIPTION'),
                    EndNote(); ?>
            </td>
        </tr>
        ...
        <tr>
            <td width="30%" class="trustedcryptoarmdocs_opt_multiline_cell"> <?= Loc::getMessage(
                'TR_CA_DOCS_EVENTS_SIGNED_BY_CLIENT'
            ) ?> </td>
            <td width="70%">
                <select name="EVENT_SIGNED_BY_CLIENT" id="EVENT_SIGNED_BY_CLIENT">
                    <option value="" <?= $EVENT_SIGNED_BY_CLIENT ? '' : 'selected' ?>>
                        <?= Loc::getMessage('TR_CA_DOCS_EVENTS_DO_NOTHING') ?>
                    </option>
        ...
        <tr>
            ...
            <td>
                <select name="EVENT_SIGNED_BY_SELLER" id="EVENT_SIGNED_BY_SELLER">
                    <option value="" <?= $EVENT_SIGNED_BY_SELLER ? '' : 'selected' ?>>
                        <?= Loc::getMessage('TR_CA_DOCS_EVENTS_DO_NOTHING') ?>
                    </option>
                    <?php foreach ($orderStatuses as $status) {
                        $statusId = htmlspecialcharsbx($status['ID']);
        ...
        <tr>
            ...
            <td>
                <select name="EVENT_SIGNED_BY_BOTH" id="EVENT_SIGNED_BY_BOTH">
                    <option value="" <?= $EVENT_SIGNED_BY_BOTH ? '' : 'selected' ?>>
                        <?= Loc::getMessage('TR_CA_DOCS_EVENTS_DO_NOTHING') ?>
                    </option>
                    <?php foreach ($orderStatuses as $status) {
                        $statusId = htmlspecialcharsbx($status['ID']);
        ...
        <tr>
            ...
            <td>
                ...
                <option value="" <?= $EVENT_EMAIL_SENT ? '' : 'selected' ?>>
                    <?= Loc::getMessage('TR_CA_DOCS_EVENTS_DO_NOTHING') ?>
                </option>
                <?php foreach ($orderStatuses as $status) {
                    $statusId = htmlspecialcharsbx($status['ID']);
        ...
        <tr>
            ...
            <td>
                ...
                <option value="" <?= $EVENT_EMAIL_READ ? '' : 'selected' ?>>
                    <?= Loc::getMessage('TR_CA_DOCS_EVENTS_DO_NOTHING') ?>
                </option>
                    <?php foreach ($orderStatuses as $status) {
                        $statusId = htmlspecialcharsbx($status['ID']);
        ...
        <tr>
            <td colspan="2">
                <?php echo BeginNote(),
                    Loc::getMessage('TR_CA_DOCS_EMAIL_DESCRIPTION'),
                    EndNote(); ?>
            </td>
        </tr>
        ...
        <tr>
            ...
            <td>
                <select name="MAIL_EVENT_ID" id="MAIL_EVENT_ID">
                    <option value="" <?= $MAIL_EVENT_ID ? '' : 'selected' ?>>
                        <?= Loc::getMessage('TR_CA_DOCS_EMAIL_NOT_SELECTED') ?>
                    </option>
                    <?php
                    $events = CEventType::GetList(array('LID' => LANGUAGE_ID), $order = 'TYPE_ID');
                    while ($event = $events->Fetch()) {
        ...
        <tr>
            ...
            <td>
                <select name="MAIL_TEMPLATE_ID" id="MAIL_TEMPLATE_ID">
                    <option value="" <?= $MAIL_TEMPLATE_ID ? '' : 'selected' ?>>
                        <?= Loc::getMessage('TR_CA_DOCS_EMAIL_NOT_SELECTED') ?>
                    </option>
                    <?php
                    $templates = CEventMessage::GetList($by = 'id', $order = 'asc', array(
                        'TYPE_ID' => $MAIL_EVENT_ID,
                    ));
                    while ($template = $templates->Fetch()) {
                        $templateId = htmlspecialcharsbx($template['ID']);
                        $templateSubject = htmlspecialcharsbx($template['SUBJECT']);
    ...
    <tr>
        <td colspan="2">
            <?php echo BeginNote(),
                Loc::getMessage('TR_CA_DOCS_FORM_EMAIL_DESCRIPTION'),
                EndNote(); ?>
        </td>
    </tr>
    ...
    <tr>
        ...
        <td width="60%">
            <select name="MAIL_EVENT_ID_FORM" id="MAIL_EVENT_ID_FORM">
                <option value="" <?= $MAIL_EVENT_ID_FORM ? '' : 'selected' ?>>
                    <?= Loc::getMessage('TR_CA_DOCS_FORM_EMAIL_NOT_SELECTED') ?>
                </option>
                <?php
                $events = CEventType::GetList(array('LID' => LANGUAGE_ID), $order = 'TYPE_ID');
                while ($event = $events->Fetch()) {
    ...
    <tr>
        ...
        <td>
            <select name="MAIL_TEMPLATE_ID_FORM" id="MAIL_TEMPLATE_ID_FORM">
                <option value="" <?= $MAIL_TEMPLATE_ID_FORM ? '' : 'selected' ?>>
                    <?= Loc::getMessage('TR_CA_DOCS_FORM_EMAIL_NOT_SELECTED') ?>
                </option>
                <?php
                $templates = CEventMessage::GetList($by = 'id', $order = 'asc', array(
                    'TYPE_ID' => $MAIL_EVENT_ID_FORM,
                ));
                while ($template = $templates->Fetch()) {
                    $templateId = htmlspecialcharsbx($template['ID']);
                    $templateSubject = htmlspecialcharsbx($template['SUBJECT']);
    ...
    <tr>
        <td colspan="2">
            <?php echo BeginNote(),
                Loc::getMessage('TR_CA_DOCS_FORM_TO_ADMIN_EMAIL_DESCRIPTION'),
                EndNote(); ?>
        </td>
    </tr>
    ...
    <tr>
        ...
        <td width="60%">
            <select name="MAIL_EVENT_ID_FORM_TO_ADMIN" id="MAIL_EVENT_ID_FORM_TO_ADMIN">
                <option value="" <?= $MAIL_EVENT_ID_FORM_TO_ADMIN ? '' : 'selected' ?>>
                    <?= Loc::getMessage('TR_CA_DOCS_FORM_TO_ADMIN_EMAIL_NOT_SELECTED') ?>
                </option>
                <?php
                $events = CEventType::GetList(array('LID' => LANGUAGE_ID), $order = 'TYPE_ID');
                while ($event = $events->Fetch()) {
    ...
    <tr>
        ...
        <td>
            <select name="MAIL_TEMPLATE_ID_FORM_TO_ADMIN" id="MAIL_TEMPLATE_ID_FORM_TO_ADMIN">
                <option value="" <?= $MAIL_TEMPLATE_ID_FORM_TO_ADMIN ? '' : 'selected' ?>>
                    <?= Loc::getMessage('TR_CA_DOCS_FORM_TO_ADMIN_EMAIL_NOT_SELECTED') ?>
                </option>
                <?php
                $templates = CEventMessage::GetList($by = 'id', $order = 'asc', array(
                    'TYPE_ID' => $MAIL_EVENT_ID_FORM_TO_ADMIN,
                ));
                while ($template = $templates->Fetch()) {
                    $templateId = htmlspecialcharsbx($template['ID']);
                    $templateSubject = htmlspecialcharsbx($template['SUBJECT']);
    ...
    if (file_exists(TR_CA_DOCS_LOG_FILE)) { ?>
        ...
        <pre><?php print_r(Docs\Utils::tail(TR_CA_DOCS_LOG_FILE, 100)); ?></pre>
        <input name="download_logs" type="submit" value="<?= Loc::getMessage(
            'TR_CA_DOCS_LOGS_DOWNLOAD'
        ) ?>" style="margin-right:5px;"/>
        <input name="purge_logs" type="submit" value="<?= Loc::getMessage(
            'TR_CA_DOCS_LOGS_PURGE'
        ) ?>"/>
    <?php } else {echo Loc::getMessage('TR_CA_DOCS_LOGS_NO_LOG_FILE');}
    ?>
    ...
    <script>
        function createAccountNumber () {
            ...
            createNewAccountNumber((data) =>  {
                ...
                alert('<?= GetMessage(
                    'TR_CA_DOCS_LICENSE_CREATE_NEW_ACCOUNT_NUMBER_ALERT'
                ) ?>' + data + '<?= GetMessage(
    'TR_CA_DOCS_LICENSE_CREATE_NEW_ACCOUNT_NUMBER_ALERT2'
) ?>');
        ...
        function editAccountNumber() {
            if (confirm('<?= Loc::getMessage(
                'TR_CA_DOCS_LICENSE_SUBMIT_DELETE_ACCOUNT_NUMBER'
            ) ?>')) {
                document.getElementById("LICENSE_ACCOUNT_NUMBER").value = "";
                        ...
                        onsuccess: function (response) {
                            ...
                            if (!res.success) {
                                infoMessage = '<?= GetMessage(
                                    'TR_CA_DOCS_LICENSE_ACTIVATE_JWT_ERROR'
                                ) ?>';
                            }
                            if (res.data.amount) {
                                infoMessage = '<?= GetMessage(
                                    'TR_CA_DOCS_LICENSE_ACTIVATE_JWT_SUCCESS'
                                ) ?>' + res.data.amount + '<?= GetMessage(
    'TR_CA_DOCS_LICENSE_ACTIVATE_JWT_SUCCESS2'
) ?>';
                                checkAccountBalance();
                            } else {
                                switch (res.data) {
                                    case 1000:
                                        infoMessage = '<?= GetMessage(
                                            'TR_CA_DOCS_LICENSE_ACTIVATE_JWT_ACCOUNT_DOES_NOT_EXIST'
                                        ) ?>';
                                        break;
                                    case 1001:
                                        infoMessage = '<?= GetMessage(
                                            'TR_CA_DOCS_LICENSE_ACTIVATE_JWT_EMPTY'
                                        ) ?>';
                                        break;
                                    case 1002:
                                        infoMessage = '<?= GetMessage(
                                            'TR_CA_DOCS_LICENSE_ACTIVATE_JWT_ALREADY_ACTIVATED'
                                        ) ?>';
                                        break;
                                    case 1003:
                                        infoMessage = '<?= GetMessage(
                                            'TR_CA_DOCS_LICENSE_ACTIVATE_JWT_FORMAT_ERROR'
                                        ) ?>';
                                        break;
                                    default:
                                        infoMessage = '<?= GetMessage(
